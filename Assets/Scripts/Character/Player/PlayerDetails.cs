...
public class PlayerDetails : MonoBehaviour
{
    ...
    [Space]
    [Header(("Bonus Stats"))]
    [SerializeField] private BonusByEquipments bonusByEquipments;
    [SerializeField] private BonusBySkills bonusBySkills;
    ...
    private void Start()
    {
        ...
        bonusByEquipments = GetComponent<BonusByEquipments>();
    ...
    private void StatFunction()
    {
        final_AttackRange = equipment.attackRange;
        // Point Stat
        final_Strength = Cal_PointStat(saoClassStartingStat.strength, bonusByEquipments.total_Strength, 0);
        final_Dexterity = Cal_PointStat(saoClassStartingStat.dexteriry, bonusByEquipments.total_Dexterity, 0);
        final_Vitality = Cal_PointStat(saoClassStartingStat.vitality, bonusByEquipments.total_Vitality, 0);
        final_Energy = Cal_PointStat(saoClassStartingStat.energy, bonusByEquipments.total_Energy, 0);
        // Life and Mana
        final_Life = Cal_LifeOrMana(saoClassStartingStat.life, current_Level, saoClassStartingStat.lifeByLevel, final_Strength, saoClassStartingStat.lifeByPoint, bonusByEquipments.total_Life, 0);
        final_Mana = Cal_LifeOrMana(saoClassStartingStat.mana, current_Level, saoClassStartingStat.manaByLevel, final_Strength, saoClassStartingStat.manaByPoint, bonusByEquipments.total_Mana, 0);
        // Damage
        final_MinDamage = Cal_Damage(equipment.minDamage, final_Strength, bonusByEquipments.total_MinDamage, damageBonusBySkill);
        final_MaxDamage = Cal_Damage(equipment.maxDamage, final_Strength, bonusByEquipments.total_MaxDamage, damageBonusBySkill);
        // Other
        final_AttackRange = equipment.attackRange;
        final_AttackSpeed = 150 + equipment.attackSpeed;
        final_AttackRating = Cal_AttackRating(final_Dexterity, bonusByEquipments.total_AttackRating, 0);
        final_Defense = Cal_Defense(final_Dexterity, bonusByEquipments.total_Defense, 0);
    }
    // Point Stat
